...
var FILES_SRC = './src',
  ...
  FILE_BOOTSTRAP = FILES_SRC + '/bootstrap.js',
  ...
  FILES_JS = [FOLDER_JS + '/*.js', FOLDER_JS + '/**/*.js', '!' + FOLDER_JS + '/libs/*'],
  ...
  FOLDER_TARGET = './dist',
  FILES_ALL_COMPILED = [FOLDER_TARGET + '/**', '!.git'],
...
gulp.task('static', ['bootstrap'], compileStatic(FILE_INDEX, isProduction, FOLDER_TARGET));
gulp.task('static-rev', ['bootstrap', 'rev'], compileStatic(FILE_INDEX, isProduction, FOLDER_TARGET));
...
gulp.task('watch', ['scripts-w', 'fonts', 'images', 'styles', 'static', 'server'], function() {
  ...
  gulp.watch(FILE_BOOTSTRAP, ['bootstrap']);
  livereload.listen();
...
/** Compiles js for bookmarklet injection into Telenor CMS. */
function bootstrap(file, isProduction, targetFolder) {
  return function() {
